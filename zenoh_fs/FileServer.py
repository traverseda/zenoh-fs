...
class FileServer:
    ...
    def handle_read(self, query):
        ...
        if method == "ticket":
            query.reply(query.key_expr, payload=self.generate_ticket(file_path, "read").encode())
            return
        ...
        if method == "list":
            ...
            logger.debug(f"Listing directory at {dir_path}")
        ...
        if method == "read":
            ...
            logger.info(f"Reading file at {read_path.absolute()} with parameters: start={start}, end={end}, chunk_size={chunk_size}")
